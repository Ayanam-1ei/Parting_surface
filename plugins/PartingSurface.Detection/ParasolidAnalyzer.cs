...
{
    /// <summary>
    /// Specification for a single Parasolid partition to analyze.
    /// </summary>
    public class PartitionSpec
    {
        public int Id;
        public string Label;
        public string XbPath;

        public PartitionSpec() { }

        public PartitionSpec(int id, string label, string xbPath)
        {
            Id = id;
            Label = label;
            XbPath = xbPath;
        ...
        {
            return Id + "|" + (Label ?? string.Empty) + "|" + (XbPath ?? string.Empty);
        }
    }

    /// <summary>
    /// Parasolid geometry analyzer. Uses PK API to extract body/face/edge topology
    /// from .x_b partition files and writes TSV output.
    /// Refactored from static class with Main() to public class with public Analyze method.
    /// Keeps all PK API calls and frustrum callbacks.
    /// </summary>
    public unsafe class ParasolidAnalyzer
    {
        ...
        // Frustrum callback delegates (must be kept as static fields to prevent GC)
        ...
        /// Usage: ParasolidAnalyzer &lt;output-directory&gt; &lt;partition-id|label|x_b-path&gt; [...]
        /// </summary>
        public static int Main(string[] args)
        {
            ...
            List<PartitionSpec> partitions = new List<PartitionSpec>();
            for (int i = 1; i < args.Length; i++)
            {
                string[] fields = args[i].Split(new[] { '|' }, 3);
                if (fields.Length != 3)
                {
                    Console.Error.WriteLine("Invalid partition specification: " + args[i]);
                    return 2;
                }
                int partitionId;
                if (!int.TryParse(fields[0], NumberStyles.Integer, CultureInfo.InvariantCulture, out partitionId))
                {
                    Console.Error.WriteLine("Invalid partition id: " + fields[0]);
                    return 2;
                }
                partitions.Add(new PartitionSpec(partitionId, fields[1], fields[2]));
            }

            ParasolidAnalyzer analyzer = new ParasolidAnalyzer();
            return analyzer.Analyze(outputDirectory, partitions);
        }

        /// <summary>
        /// Analyze the given partitions and write TSV output to outputDir.
        /// Returns 0 on success, non-zero on error.
        /// </summary>
        public int Analyze(string outputDir, List<PartitionSpec> partitions)
        {
            ...
            {
                throw new ArgumentException("partitions must not be empty", "partitions");
            }

            string outputDirectory = Path.GetFullPath(outputDir);
            ...
            try
            {
                ...
                {
                    LoadPartition(partitions[i], bodies);
                }
        ...
        private static void LoadPartition(PartitionSpec spec, List<BodyEntry> bodies)
        {
            int partitionId = spec.Id;
            string partitionLabel = spec.Label ?? string.Empty;
            string path = Path.GetFullPath(spec.XbPath);
            ...
            try
            {
                for (int partIndex = 0; partIndex < partCount; partIndex++)
                {
                    ...
                    entry.PartitionLabel = partitionLabel;
        ...
        // --- Frustrum callbacks (static, called from Parasolid native code) ---
